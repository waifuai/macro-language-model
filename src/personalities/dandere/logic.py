...
from personalities.dandere.data import dandere_responses, dandere_default_responses
import random
...
class DandereLogic(PersonalityInterface):
    ...
    def introduce_topic(self, topic: str, context: Dict[str, Any]) -> Optional[str]:
        ...
        # Dandere-specific topic introductions
        introductions = {
            ...
            "interest_manga": "U-um... do you like manga...?",
            "interest_anime": "I-I'm a little shy, but... do you watch anime...?",
            "interest_games": "D-do you... um... play any games...?",
            "interest_cooking": "S-sorry... do you like to cook...?",
        ...
        return introductions.get(topic, f"U-um... L-let's talk about {topic}...?")

    def get_default_response(self, context: Dict[str, Any]) -> str:
        """Gets a default response."""
        if context.get("debug"):
            print("DandereLogic: Getting default response")

        used_responses = context["conversation_context"].used_responses
        unused_responses = [resp for resp in dandere_default_responses if resp not in used_responses]
    ...
    def get_data(self) -> Dict[str, Any]:
        ...
        return {"responses": dandere_responses, "default_responses": dandere_default_responses}
    ...
    def handle_transformations(self, transformations: Dict[str, Tuple[Any, Optional[str], int]], input_list: List[str], waifu_memory: Any, talk_about_interest: Callable[..., str], introduce_topic: Callable[..., str], debug: bool) -> Optional[str]:
        ...
        return apply_transformations(transformations, input_list, waifu_memory, "dandere", talk_about_interest, introduce_topic, lambda context, *responses: self.get_default_response(context), {}, set(), [], debug, self)
    ...
    def maybe_introduce_topic(self, topic_manager: Any, input_str: str, turn_count: int) -> Optional[str]:
        ...
        if topic_manager.waifu_chatbot.debug:
            print(f"DandereLogic.maybe_introduce_topic: Entering with input: {input_str}, turn: {turn_count}")
    ...
    def respond_based_on_current_topic(self, topic_manager: Any, tokens: List[str], keywords: Dict[str, List[Tuple[str, Any]]], context: Dict[str, Any], response_templates: Dict[tuple[str, str], List[str]]) -> Optional[str]:
        ...
        if topic_manager.waifu_chatbot.debug:
            print(f"DandereLogic.respond_based_on_current_topic: Entering with tokens: {tokens}")
        if topic_manager.current_topic:
            if topic_manager.waifu_chatbot.debug:
                print(f"DandereLogic.respond_based_on_current_topic: Current topic: {topic_manager.current_topic}, topic_turns: {topic_manager.topic_turns}")
            ...
            for token in tokens:
                ...
                if token == topic_manager.current_topic or (topic_manager.current_topic in topic_manager.waifu_chatbot.registry.keywords and token in topic_manager.waifu_chatbot.registry.keywords and topic_manager.waifu_chatbot.registry.keywords.get(token) == topic_manager.waifu_chatbot.registry.keywords.get(topic_manager.current_topic)):
                    ...
                    break
            print(f"DandereLogic.respond_based_on_current_topic: input_relates_to_topic = {input_relates_to_topic}")  # DEBUG
            ...
            if topic_manager.topic_turns > 0:  # Only respond if topic_turns > 0
                if topic_manager.current_topic in keywords:
                    for resp_pattern, resp_text in keywords[topic_manager.current_topic]:
                        if matches(tokenize(resp_pattern), tokens):
                            ...
                            if topic_manager.waifu_chatbot.debug:
                                print(f"DandereLogic.respond_based_on_current_topic: Found response: {resp_text}")
                            return resp_text

                # If we have a current_topic, but no specific keyword was found:
                # Dandere-specific responses based on topic
                if topic_manager.current_topic in dandere_responses:
                    responses = dandere_responses[topic_manager.current_topic]
                ...
                if topic_manager.waifu_chatbot.debug:
                    print(f"DandereLogic.respond_based_on_current_topic: Before decrement, topic_turns = {topic_manager.topic_turns}")
                topic_manager.topic_turns -= 1
                print(f"DandereLogic.respond_based_on_current_topic: topic_turns after decrement: {topic_manager.topic_turns}")  # DEBUG
                if topic_manager.waifu_chatbot.debug:
                    print(f"DandereLogic.respond_based_on_current_topic: After decrement, topic_turns = {topic_manager.topic_turns}")
                ...
                if topic_manager.waifu_chatbot.debug:
                    print(f"DandereLogic.respond_based_on_current_topic: No more topic turns remaining")
        ...
        if topic_manager.topic_turns <= 0 or not input_relates_to_topic:
            if topic_manager.waifu_chatbot.debug:
                print("DandereLogic.respond_based_on_current_topic: Exiting topic due to no engagement or turns limit")
    ...
    def talk_about_interest(self, waifu_memory: Any, current_dere: str, used_responses: List[str], debug: bool) -> str:
        """Generates a dandere-specific response about a random interest."""
        interests = {
            "reading manga": "U-um, I really like this manga... I-I hope you like it too... I-it's a little embarrassing, but this manga is kind of special to me...",
            "watching anime": "U-um, I really enjoyed this anime... M-maybe we could watch it together sometime...? I-it's a little embarrassing, but this anime made me cry a little...",
            "playing video games": "U-um, I really like this game... M-maybe we could play it together sometime...? I-it's a little embarrassing, but I'm kind of addicted to this game...",
            "cooking": "U-um, I made this                food... I-I hope you like it... I-it's a little embarrassing, but I put a lot of effort into making this...",
        }
        if not waifu_memory.interests:
            return "U-um... I-I'm sorry..."  # Dandere-style default response

        interest = random.choice(waifu_memory.interests)
        return interests.get(interest, "U-um... I-I don't know...")