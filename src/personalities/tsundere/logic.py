...
from personalities.tsundere.data import tsundere_responses, tsundere_default_responses
import random
...
class TsundereLogic(PersonalityInterface):
    ...
    def introduce_topic(self, topic: str, context: Dict[str, Any]) -> Optional[str]:
        ...
        # Tsundere-specific topic introductions
        introductions = {
            ...
            "interest_manga": "I-It's not like I want to know, but what manga are you reading?",
            "interest_anime": "Baka! Don't think I care, but what anime are you watching?",
            "interest_games": "Hmph. I bet you're not very good, but what games do you play?",
            "interest_cooking": "It's not like I'll cook for you, but what do you like to cook?",
        ...
        return introductions.get(topic, f"Let's talk about {topic}. Not that I care, baka!")

    def get_default_response(self, context: Dict[str, Any]) -> str:
        """Gets a default response."""
        if context.get("debug"):
            print("TsundereLogic: Getting default response")

        used_responses = context["conversation_context"].used_responses
        unused_responses = [resp for resp in tsundere_default_responses if resp not in used_responses]
    ...
    def get_data(self) -> Dict[str, Any]:
        ...
        return {"responses": tsundere_responses, "default_responses": tsundere_default_responses}
    ...
    def handle_transformations(self, transformations: Dict[str, Tuple[Any, Optional[str], int]], input_list: List[str], waifu_memory: Any, talk_about_interest: Callable[..., str], introduce_topic: Callable[..., str], debug: bool) -> Optional[str]:
        ...
        return apply_transformations(transformations, input_list, waifu_memory, "tsundere", talk_about_interest, introduce_topic, lambda context, *responses: self.get_default_response(context), {}, set(), [], debug, self)
    ...
    def maybe_introduce_topic(self, topic_manager: Any, input_str: str, turn_count: int) -> Optional[str]:
        ...
        if topic_manager.waifu_chatbot.debug:
            print(f"TsundereLogic.maybe_introduce_topic: Entering with input: {input_str}, turn: {turn_count}")
    ...
    def respond_based_on_current_topic(self, topic_manager: Any, tokens: List[str], keywords: Dict[str, List[Tuple[str, Any]]], context: Dict[str, Any], response_templates: Dict[tuple[str, str], List[str]]) -> Optional[str]:
        ...
        if topic_manager.waifu_chatbot.debug:
            print(f"TsundereLogic.respond_based_on_current_topic: Entering with tokens: {tokens}")
        if topic_manager.current_topic:
            if topic_manager.waifu_chatbot.debug:
                print(f"TsundereLogic.respond_based_on_current_topic: Current topic: {topic_manager.current_topic}, topic_turns: {topic_manager.topic_turns}")
            ...
            for token in tokens:
                ...
                if token == topic_manager.current_topic or (topic_manager.current_topic in topic_manager.waifu_chatbot.registry.keywords and token in topic_manager.waifu_chatbot.registry.keywords and topic_manager.waifu_chatbot.registry.keywords.get(token) == topic_manager.waifu_chatbot.registry.keywords.get(topic_manager.current_topic)):
                    ...
                    break
            print(f"TsundereLogic.respond_based_on_current_topic: input_relates_to_topic = {input_relates_to_topic}")  # DEBUG
            ...
            if topic_manager.topic_turns > 0:  # Only respond if topic_turns > 0
                if topic_manager.current_topic in keywords:
                    for resp_pattern, resp_text in keywords[topic_manager.current_topic]:
                        if matches(tokenize(resp_pattern), tokens):
                            ...
                            if topic_manager.waifu_chatbot.debug:
                                print(f"TsundereLogic.respond_based_on_current_topic: Found response: {resp_text}")
                            return resp_text

                # If we have a current_topic, but no specific keyword was found:
                # Tsundere-specific responses based on topic
                if topic_manager.current_topic in tsundere_responses:
                    responses = tsundere_responses[topic_manager.current_topic]
                ...
                if topic_manager.waifu_chatbot.debug:
                    print(f"TsundereLogic.respond_based_on_current_topic: Before decrement, topic_turns = {topic_manager.topic_turns}")
                topic_manager.topic_turns -= 1
                print(f"TsundereLogic.respond_based_on_current_topic: topic_turns after decrement: {topic_manager.topic_turns}")  # DEBUG
                if topic_manager.waifu_chatbot.debug:
                    print(f"TsundereLogic.respond_based_on_current_topic: After decrement, topic_turns = {topic_manager.topic_turns}")
                ...
                if topic_manager.waifu_chatbot.debug:
                    print(f"TsundereLogic.respond_based_on_current_topic: No more topic turns remaining")
        ...
        if topic_manager.topic_turns <= 0 or not input_relates_to_topic:
            if topic_manager.waifu_chatbot.debug:
                print("TsundereLogic.respond_based_on_current_topic: Exiting topic due to no engagement or turns limit")
    ...
    def talk_about_interest(self, waifu_memory: Any, current_dere: str, used_responses: List[str], debug: bool) -> str:
        """Generates a tsundere-specific response about a random interest."""
        interests = {
            "reading manga": "You know, I've been reading this really great manga lately. It's about a magical girl fighting evil. You should check it out! ...Not that I care if you read it or not. B-baka!",
            "watching anime": "I just finished watching this amazing anime. It's a slice-of-life anime with beautiful animation. I cried at the end, not that I would admit that normally. ...Don't get any weird ideas just because I shared this with you.",
            "playing video games": "I'm so addicted to this new game! It's a JRPG where you have to collect rare items. Wanna play with me sometime? ...But don't think this means I'll go easy on you!",
            "cooking": "I-it's not like I made these cookies for you or anything, b-baka! I just had some extra ingredients. They're pocky flavored. ...And don't expect me to cook for you again!",
        }
        if not waifu_memory.interests:
            return "What are you talking about? Baka!"  # Tsundere-style default response

        interest = random.choice(waifu_memory.interests)
        return interests.get(interest, "Hmph. I don't have any interests. Not that I'd tell you anyway.")